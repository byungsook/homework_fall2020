...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        if self.gamma < 1:
            gamma = np.power(self.gamma, np.arange(len(rewards)))
            rewards *= gamma
        discounted_return = np.sum(rewards)
        ...
        if self.gamma < 1:
            ...
            for r in reversed(rewards[:-1]):
                list_of_discounted_cumsums.append(
                    r + list_of_discounted_cumsums[-1] * self.gamma
                )
        else:
            list_of_discounted_cumsums = np.cumsum(rewards[::-1])
